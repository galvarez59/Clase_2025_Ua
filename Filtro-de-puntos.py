    ...
    class AreaUI:
        ...
        def _graficar(self, df, area=None, titulo="Puntos", mantener_limites=False):
            ...
            if df is not None and not df.empty:
                self.ax.scatter(df["X"], df["Y"], s=28, alpha=0.82, c="#a474a5", edgecolors="#721b5e", linewidths=0.7, zorder=3)
                color_puntos = "#1c8234" if area else "#1d577e"  # Verde si es filtrado, azul si no
                self.ax.scatter(df["X"], df["Y"], s=28, alpha=0.82, c=color_puntos,
                        edgecolors="#133850", linewidths=0.7, zorder=5, label="Área")
